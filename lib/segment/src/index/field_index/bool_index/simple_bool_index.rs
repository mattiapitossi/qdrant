...
mod memory {
    ...
    impl BoolMemory {
        ...
        pub fn iter_has_true_measured(
            &self,
            hw_acc: HwMeasurementAcc,
        ) -> impl Iterator<Item = PointOffsetType> + '_ {
            self.trues
                .iter_ones()
                .measure_hw_with_acc(hw_acc, size_of::<usize>(), |i| {
        ...
        pub fn iter_has_false_measured(
            &self,
            hw_acc: HwMeasurementAcc,
        ) -> impl Iterator<Item = PointOffsetType> + '_ {
            self.falses
...
impl PayloadFieldIndex for SimpleBoolIndex {
    ...
    fn filter<'a>(
        ...
        hw_acc: HwMeasurementAcc, // TODO(io_measurement): Measure this index
        ...
        match &condition.r#match {
            Some(Match::Value(MatchValue {
                ...
                if *value {
                    Some(Box::new(self.memory.iter_has_true_measured(hw_acc)))
                } else {
                    Some(Box::new(self.memory.iter_has_false_measured(hw_acc)))
                }
            }
            _ => None,
        }
    ...
    fn estimate_cardinality(
        ...
        match &condition.r#match {
            Some(Match::Value(MatchValue {
                ...
                let memory_len;
                let count = if *value {
                    memory_len = self.memory.trues_count();
                    self.memory.trues_count()
                } else {
                    memory_len = self.memory.falses_count();
                    self.memory.falses_count()
                };

                hw_counter
                    .payload_index_io_read_counter()
                    .incr_delta(memory_len);
